...
def search(myFile):
    ...
    found = False
    while not found:
        modifiedRound = remove_floor(currentRound)
        nextRound = get_next_round(modifiedRound)
...
def remove_floor(myFile):
    outputFile = []
    for j in range(len(myFile)):
        print("input=" + myFile[j])
        currentLine = ''
        for i in range(len(myFile[0])):
            if myFile[j][i] != '.':
                currentLine += myFile[j][i]

        while len(currentLine) != len(myFile[0]):
            currentLine += '0'
