...
def find_grid_to_win_last(grids):
    return 1
...
def is_game_finished(grids,fullBoards):
    for i in range(0, len(grids)):
        if is_line_full(grids[i]) or is_colonne_full(grids[i]):
            if i not in fullBoards:
                fullBoards.append(i)
    return fullBoards
...
def main(fichier):
    ...
    numberId = 0
    fullBoards = []
    while (len(fullBoards) < len(grids)):
        currentNumber = numberDrawList[numberId]
        for i in range(0, len(grids)):
            grids[i] = update_grid(grids[i], currentNumber)
        fullBoards = is_game_finished(grids,fullBoards)
        numberId += 1

    return sum_of_unmarked(grids[fullBoards[-1]]) * numberDrawList[numberId -1]
