...
def compute_cells_to_update(vent):
    ...
    if is_diag_line(vent):
        ...
        for i in range(x1, x2+1):
            updateList.append((i,y1+(direction*j)))
            j += 1
...
def is_diag_line(vent):
    return (abs(vent[0][0]-vent[1][0])) == (abs(vent[0][1]-vent[1][1]))
    # return ((vent[0][0] == vent[0][1]) and (vent[1][0] == vent[1][1])) or ((vent[0][0] == vent[1][1]) and (vent[1][0] == vent[0][1]))
